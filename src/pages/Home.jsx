...
const Home = () => {
  ...
  const healthyFoods = [
    {
      id: 101, // IDs unique rakhi hain taaki ExploreMenu se clash na ho
      ...
      price: 342, // Numbers mein rakha hai calculation ke liye
    },
    {
      id: 102,
  ...
  // Reusable Food Card Component (Internal rakha hai taaki props pass ho sakein)
  const FoodCard = ({ food }) => (
    <div className="bg-white rounded-3xl p-5 border border-green-100 shadow-sm hover:shadow-lg transition-shadow duration-300 flex flex-col items-center text-center">
      ...
      <div className="flex items-center justify-between w-full mt-2 pt-2 border-t border-gray-100">
        ...
        {/* 3. Button pe addToCart lagaya */}
        <button 
          onClick={() => addToCart(food)}
          className="bg-green-600 hover:bg-green-700 text-white text-xs px-4 py-2 rounded-full font-semibold transition-all active:scale-95 shadow-md"
        >
          Add
        </button>
      </div>
